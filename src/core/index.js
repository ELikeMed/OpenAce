...
// ═══ Phase 4: SuperBrain upgrades ═══
  ...
  async chat(userMessage, options = {}) {
    ...
    try {
      ...
      if (this.brain) {
        ...
        // TEACH marker — learn new SOP
        if (brainResult.data?._teachMarker) {
          const newSOP = await this.learnNewSOP(userMessage);
          if (newSOP) {
            const msg = `🧠 **Got it! I've learned a new procedure:**\n\n**${newSOP.name}**\n${newSOP.description}\n\n**Triggers:** ${newSOP.triggers.map(t => `"${t}"`).join(', ')}\n\n**Steps:** ${newSOP.steps.length} steps saved\n\nNext time you say any of those triggers, I'll execute this automatically!\n\nYou can view/edit this in the **SOPs / Training** section.`;

            this.conversationHistory.push({
              role: 'assistant', content: msg, timestamp: new Date().toISOString(),
              metadata: { type: 'sop_created', sopId: newSOP.id }
            });
            return { message: msg, action: 'sop_created', data: newSOP,
              metadata: { type: 'sop_created', sopId: newSOP.id, timestamp: new Date().toISOString() } };
          }
        }
  ...
  /**
   * Learn a new SOP from user's natural language description
   * This is Ace's SMART LEARNING capability!
   */
  async learnNewSOP(userMessage) {
    this.onProgress('🧠 Learning new procedure...');
    
    try {
      // Use AI to parse the teaching into structured SOP
      const parsePrompt = `You are an SOP (Standard Operating Procedure) parser. You convert human instructions into executable automation steps.

User's teaching message:
"""
${userMessage}
"""

CRITICAL RULES:
1. NEVER create a click_text step for section headings, labels, or instructional text like "Step 1:", "Note:", "If found:", "Exception Handling", "Final Review". These are instructions for YOU, not things to click.
2. URLs in navigate steps MUST be real, valid URLs (https://...). Never use placeholder URLs like "https://the listing image".
3. Always add {"action": "wait", "ms": 3000} AFTER every navigate step — pages need time to load.
4. Use {{variable}} placeholders for dynamic values the user will provide at runtime (e.g., {{city}}, {{price}}, {{date}}).
5. For editing existing text in a field (removing prefix, changing value), use "edit_field" — NOT "type".
6. For setting/changing dates, use "set_date" — NOT "type" or "click_text".
7. For multi-step processes with conditions ("if X, then Y"), use "decide" to let AI choose at runtime.
8. Triggers should be short natural phrases a user would say to start this task. Keywords should be meaningful words only (no: the, a, an, to, for, on, in, of, we, any, how, is, it, and, or, but, if, at, by, with, from, that, this, they, them).

AVAILABLE ACTIONS (use the right one for each instruction):

NAVIGATION:
- {"action": "navigate", "url": "https://real-url.com", "description": "Go to site"}
- {"action": "search_google", "query": "search terms", "description": "Google search"}

CLICKING:
- {"action": "click_text", "text": "visible button/link text", "description": "Click element with this text"}
- {"action": "click_submit", "description": "Click the submit/save button"}
- {"action": "smart_click", "target": "description of what to click", "description": "AI finds and clicks element"}

TYPING & FORMS:
- {"action": "type", "text": "text to type", "description": "Type into focused/found field"}
- {"action": "edit_field", "target": "field name or label", "remove": "text to remove", "add": "text to add", "description": "Edit existing field value"}
- {"action": "set_date", "target": "Start Date|End Date", "value": "{{date_variable}}", "description": "Set a date picker field"}
- {"action": "fill_form", "fields": {"Name": "value", "Email": "value"}, "description": "Fill multiple form fields"}
- {"action": "fill_credentials", "credentialId": "site_name", "description": "Auto-fill saved login credentials"}
- {"action": "smart_fill", "target": "field description", "value": "what to enter", "description": "AI finds field and fills it"}

WAITING:
- {"action": "wait", "ms": 3000, "description": "Wait for page to load"}
- {"action": "wait_for", "text": "element text to wait for", "description": "Wait until element appears"}

PAGE ANALYSIS:
- {"action": "explore_page", "text": "what to look for", "description": "Scan page for specific content"}
- {"action": "screenshot", "description": "Take a screenshot of current page"}
- {"action": "extract_text", "target": "what text to extract", "description": "Extract specific text from page"}
- {"action": "extract_results", "description": "Extract search results or listings"}

TEXT OPERATIONS:
- {"action": "copy", "target": "what to copy", "description": "Copy text from page"}
- {"action": "select", "target": "what to select", "description": "Select text on page"}

KEYBOARD:
- {"action": "press", "key": "Enter|Tab|Escape", "description": "Press a keyboard key"}
- {"action": "scroll", "direction": "down|up", "description": "Scroll the page"}

CONDITIONAL:
- {"action": "decide", "question": "What should I do here?", "options": ["option1", "option2"], "description": "AI decides based on page state"}
- {"action": "ensure_logged_in", "description": "Check if logged in, login if needed"}

Respond ONLY with a valid JSON object (no markdown, no code fences, no explanation):
{
  "name": "Short descriptive name",
  "description": "Brief summary of what this procedure does",
  "category": "general|lead_generation|email|meetups|custom",
  "triggers": ["natural phrase to trigger this", "shorter variant"],
  "keywords": ["meaningful", "words", "only"],
  "steps": [...]
}

If you cannot parse a valid SOP, return: {"error": "reason"}`;
      ...
      try {
        ...
        this.onProgress('⚠ Could not parse SOP from description');
        return null;
      }

      // Check if AI returned an error
      if (sopData.error) {
        this.onProgress(`⚠ ${sopData.error}`);
        return null;
      }

      // Validate required fields
      if (!sopData.name || !sopData.triggers || !sopData.steps) {
        this.onProgress('⚠ Missing required SOP fields');
        return null;
      }

      // Validate and clean up the parsed SOP before saving
      sopData = this._validateNewSOP(sopData);

      // Create the SOP
      const newSOP = await this.sopManager.createSOP({
        id: `sop_${Date.now()}`,
        name: sopData.name,
      ...
      this.onProgress(`✅ Learned: ${newSOP.name}`);
      return newSOP;

    } catch (error) {
      this.onProgress(`❌ Failed to learn: ${error.message}`);
      return null;
    }
  }

  /**
   * Validate and clean up AI-parsed SOP data before saving.
   * Catches common AI mistakes: heading clicks, bad URLs, stop words, missing waits.
   */
  _validateNewSOP(sopData) {
    ...
    for (let i = 0; i < sopData.steps.length; i++) {
      ...
      if (step.action === 'click_text' && step.text) {
        const isHeading = headingPatterns.some(p => p.test(step.text.trim()));
        if (isHeading) {
          issues.push(`Removed heading click: "${step.text}"`);
          continue; // Skip this step entirely
        }
      }

      // Flag invalid URLs in navigate steps
      if (step.action === 'navigate' && step.url) {
        if (!step.url.startsWith('http://') && !step.url.startsWith('https://')) {
          issues.push(`Fixed invalid URL: "${step.url}"`);
          step.url = 'https://' + step.url;
        }
        if (step.url.includes(' ') || !step.url.includes('.')) {
          issues.push(`Removed invalid navigate URL: "${step.url}"`);
          continue; // Skip steps with garbage URLs
        }
      }
    ...
    // Clean up keywords — remove stop words, deduplicate
    if (sopData.keywords && Array.isArray(sopData.keywords)) {
      ...
      // Generate keywords from name and triggers
      const allText = [sopData.name, ...(sopData.triggers || [])].join(' ').toLowerCase();
      sopData.keywords = [...new Set(
        allText.split(/\s+/).filter(w => w.length > 2 && !stopWords.has(w))
      )];
    }

    // Clean up triggers — lowercase, no empty strings
    sopData.triggers = sopData.triggers
      .map(t => t.trim().toLowerCase())
      .filter(t => t.length > 0);

    // Log issues for transparency
    if (issues.length > 0) {
      this.onProgress(`🔧 SOP cleanup: ${issues.join(', ')}`);
    }
